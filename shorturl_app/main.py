...
@app.put("/stats/{short_id}")
def upgrade(short_id: str, data: URLUpgrade, db: Session = Depends(get_db)):
    url_item = db.query(URLItem).filter(URLItem.short_id == short_id).first()
    if not url_item:
        raise HTTPException(status_code=404, detail="Ссылка отсутствует")
    url_item.full_url = str(data.full_url)
    db.commit()
    db.refresh(url_item)
    return {
        "short_id": url_item.short_id,
        "new_full_url": url_item.full_url
    }
